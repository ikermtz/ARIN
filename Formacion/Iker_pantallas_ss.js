    ...
    function (n_search, serverWidget, n_record, n_format, n_log) {
        ...
        function onRequest(context) {
            ...
            if (context.request.method == 'GET') {
                ...
                n_log.debug({title: 'Crea correctamente el movimiento'});

                // Redirección
                var form = crearPantallaInicio(context);
                context.response.writePage(form);

                n_log.debug({title: 'Redirección a la pantalla inicial'});
            }

            log.debug({ title: 'Fin onRequest' });
        }

        function crearPantallaInicio(context) {

            n_log.debug({title: 'Entra en pantalla inicial'});
            ...
            form.clientScriptFileId = getFileId('Iker_pantallas_ce.js');
            ...
            n_log.debug({title: 'Pantalla de inicio creada'});
            ...
            if(parametros.buscar == 'Y' && !!parametros.usuario) {
                ...
                if (buscarMov != undefined) {
                    ...
                    buscarMov.each(function(movi){
                        ...
                        fila = fila + 1;
                        n_log.debug({title: 'Iteracion de movimiento encontrado'});
                    });                    
                }               
            } 
        ...
        function crearPantallaSecundaria(context) {
            ...
            n_log.debug({title: 'Entra en pantalla secundaria'});
        ...
        function generarMovimiento(context) {
            ...
            n_log.debug({title: 'Entra en generar movimiento'});
            ...
            try { 
                newMov.save();
                log.debug({title: 'Nuevo movimiento creado'});
            } catch (e) {
                log.error({
                    title: e.name,
                    details: e.message
        ...
        function conseguirMovimientos(usuario, inicio, fin){   
            ...
            n_log.debug({title: 'Entra en conseguir movimientos'});
